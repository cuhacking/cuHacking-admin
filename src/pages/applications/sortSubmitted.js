import { CopyToClipboard } from 'react-copy-to-clipboard'

const URL_REGEX = /^(?:(?:(?:https?|ftp):)?\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z0-9\u00a1-\uffff][a-z0-9\u00a1-\uffff_-]{0,62})?[a-z0-9\u00a1-\uffff]\.)+(?:[a-z\u00a1-\uffff]{2,}\.?))(?::\d{2,5})?(?:[/?#]\S*)?$/i

const anonymize = user => {
  return {
    uid: user.uid,
    score: -1,
    accomplishmentStatement: user.application.skills.accomplishmentStatement,
    challengeStatement: user.application.skills.challengeStatement
  }
}

export default users => {
  let under18 = []
  let experienced = []
  let novice = []

  const submitted = users.filter(user => user.application.status === 'submitted')

  submitted.forEach(user => {
    const { application } = user

    if (application.terms.under18) {
      under18.push(anonymize(user))
    } else {
      if (application.skills.numHackathons >= 3) {
        experienced.push(anonymize(user))
      } else {
        novice.push(anonymize(user))
      }
    }
  })

  return {
    under18,
    experienced,
    novice
  }
}
